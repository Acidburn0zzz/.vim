" Maintainer: Aidan Shafran <zelbrium@gmail.com>
" Forked From: Alex Efros <powerman-asdf@ya.ru>
" Version: 1.2.2
" Last Modified: 2017-07-20
" License: This file is placed in the public domain.
" Description: Auto save/load sessions
"
" Modified so that it will (almost) always save a session file, even if
" you only have one file open (the original will erase the session upon
" closing if you only have one file open). The only case where this doesn't
" save the session, and deletes the session file, is when there is one file
" open and it has no lines or columns (eg you do :e non_existant_file before
" closing).
...
if !exists('g:autosess_dir')
	let g:autosess_dir   = '~/.vim/sessions/'
endif
...
function AutosessUpdate()
	...
	if winnr('$') != 1 || line('$') != 1 || col('$') != 1 || tabpagenr('$') != 1 || &modified
